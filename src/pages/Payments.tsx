    ...
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Track recurring and scheduled payments</p>
        </div>
        <button
      ...
      <div>
        ...
        <div className="bg-card border border-border rounded-lg divide-y divide-border">
              ...
              <div key={p.id} className="flex items-center justify-between p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{p.type} · {format(new Date(p.dueDate), "MMM d, yyyy")}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className="font-heading text-sm font-semibold text-foreground">{fmt(p.amount)}</span>
          ...
          {upcoming.length === 0 && (
            <p className="p-4 text-sm text-muted-foreground">No upcoming payments</p>
          )}
        </div>
      </div>
      ...
      {paid.length > 0 && (
        <div>
          <h2 className="font-heading text-sm font-semibold text-muted-foreground mb-3">Paid</h2>
          <div className="bg-card border border-border rounded-lg divide-y divide-border opacity-60">
            {paid.map((p) => (
              <div key={p.id} className="flex items-center justify-between p-4">
                ...
                <div className="flex items-center gap-3">
                  <span className="font-heading text-sm text-muted-foreground">{fmt(p.amount)}</span>
